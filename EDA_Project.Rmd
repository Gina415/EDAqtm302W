...
The dataset contains 32,629 observations with 630 variables, which provides information about 2024 NHIS data on survey participants.Taken only the variables, we need and discard the other variables. The new dataset(data_selected) contains only 19 variables. The variables are as follows:
...
#### well_rested
```{r}
ggplot(data_selected, aes(x = well_rested)) +
  geom_bar() +
...
#### fatigue
```{r}
ggplot(data_selected, aes(x = fatigue)) +
  geom_bar() +
...
ggplot(data_selected, aes(x = factor(mh_group,
...
table(data_selected$mh_treatment)

ggplot(data_selected, aes(x = factor(mh_treatment,
...
ggplot(data_selected, aes(x = mh_group, y = sleep_hours, fill = mh_group)) +
  geom_boxplot(outlier.alpha = 0.2) +
  labs(
    title = "Hours slept by mental health group",
    x = "Mental health group",
    y = "Hours slept") +
  theme_minimal() 
...
sleep_vars <- c("sleep_hours","well_rested",
                "trouble_fall_asleep","trouble_stay_asleep",
                "prescribe_med_sleep","otc_med_sleep","mj_sleep",
                "fatigue")
...
data_corr <- data_selected %>%
  select(all_of(c(sleep_vars, mh_vars))) %>%
